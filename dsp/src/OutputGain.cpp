...
float dbToLinear(float gainDb) noexcept
{
...
void OutputGain::reset() noexcept
{
    currentGainLinear_ = targetGainLinear_;
}

void OutputGain::setParams(const OutputGainParams& params) noexcept
{
    targetGainLinear_ = dbToLinear(params.gainDb);
}
...
void OutputGain::process(const Sample* in, Sample* out, int numSamples) noexcept
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        const float gain =
            isLastSample ? targetGainLinear_
